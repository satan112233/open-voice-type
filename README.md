...
按下 `Ctrl+Alt+F`，直接说一段中文，松手后英文译文自动出现在光标处。支持英 / 中 / 日 / 韩 / 法 / 德 / 西等多种目标语言，在设置页切换。

- 一次 LLM 调用同时完成「理解口述意图」+「地道翻译」，不是逐字直译。
- 自动识别改口、去除填充词、修正同音错字，让译文更自然。
- 复用口语优化所配置的大模型与 API Key，无需单独配置。
- 翻译失败时自动回退到识别原文，不卡流程。
...
| `Ctrl + Alt + F` | 开始 / 停止边说边翻译（应用内 / 外均可） | ✅ |
| `Enter` | 录音中确认结束并提交 | ✅ |
| `Escape` | 录音中取消并丢弃 | ✅ |

> 自定义方式：设置 → 语音输入 / 边说边翻译 → 全局热键，点击输入框后按下新组合键即可。录音开始后，也可直接按 `Enter` 结束、`Esc` 取消，无需再按一次开始热键。
